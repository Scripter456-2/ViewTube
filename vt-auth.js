 ...
 * ─────────────────────────────────────────────────
 * localStorage IS the database. JSON files are only
 * read on the very first visit to seed initial data.
 * After that every change (upload, delete, subscribe,
 * like, comment) is written directly to localStorage
 * and is reflected instantly everywhere — no reloads,
 * no version bumps, no console commands needed.
 *
 * To update your repo's JSON files after making
 * changes inside the app, use:
 *   VT.exportJSON('videos')    → downloads videos.json
 *   VT.exportJSON('channels')  → downloads channels.json
 *   VT.exportJSON('shorts')    → downloads shorts.json
 *   VT.exportJSON('comments')  → downloads comments.json
 * Then commit the downloaded file to GitHub.
 */
...
const VT = (() => {
  ...
  const KEYS = {
    ...
    SUBS:     'vt_subscriptions',
    LIKES:    'vt_likes',
    DISLIKES: 'vt_dislikes',
    VIEWS:    'vt_views',
    SEEDED:   'vt_seeded',        // flag: have we loaded JSON yet?
  };
  ...
  // ── EXPORT — download current localStorage data as a JSON file ─
  // Use this to sync changes back to your repo.
  // exportJSON('videos')   → downloads videos.json
  // exportJSON('channels') → downloads channels.json
  // exportJSON('shorts')   → downloads shorts.json
  // exportJSON('comments') → downloads comments.json
  // exportJSON('all')      → downloads all four files
  function exportJSON(which = 'videos') {
    const map = {
      videos:   { key: KEYS.VIDEOS,   file: 'videos.json' },
      channels: { key: KEYS.CHANNELS, file: 'channels.json' },
      shorts:   { key: KEYS.SHORTS,   file: 'shorts.json' },
      comments: { key: KEYS.COMMENTS, file: 'comments.json' },
    };

    const targets = which === 'all' ? Object.values(map) : [map[which]];
    if (!targets[0]) { console.error('[VT] exportJSON: unknown key', which); return; }

    targets.forEach(({ key, file }) => {
  ...
  // Hard reset — wipes all data and re-reads JSON files on next load.
  // Preserves nothing. Use only when you want a clean slate.
  function hardReset() {
    if (!confirm('This will delete ALL data including accounts. Continue?')) return;
  ...
  function requireAuth(redirect = 'login.html') {
    if (!currentUser()) { window.location.href = redirect; return false; }
    return true;
  }
  ...
  function getChannelByOwner(uid)  { return getChannels().find(c => c.ownerId === uid) || null; }
  ...
  function createChannel({ name, handle, description, avatar, banner, ownerId }) {
    ...
    if (channels.find(c => c.handle === cleanHandle))
    ...
    const ch = {
      ...
      subscribers: '0',
      subscriberCount: 0,
      IsVerified: false, IsGoldVerified: false, IsBlackVerified: false,
      IsBanned: false,
      ownerId,
      createdAt: Date.now(),
    };
    store.set(KEYS.CHANNELS, [...channels, ch]);

    // Link to user record
    const users = getUsers();
    const ui = users.findIndex(u => u.id === ownerId);
    if (ui !== -1) { users[ui].channelId = ch.id; store.set(KEYS.USERS, users); }
  ...
  function isSubscribed(uid, cid) { return getUserSubs(uid).includes(cid); }

  function toggleSubscribe(userId, channelId) {
    const subs     = getSubs();
    const list     = subs[userId] || [];
    const channels = getChannels();
    const ci       = channels.findIndex(c => c.id === channelId);
    const wasSub   = list.includes(channelId);

    subs[userId] = wasSub ? list.filter(id => id !== channelId) : [...list, channelId];
    store.set(KEYS.SUBS, subs);

    if (ci !== -1) {
      channels[ci].subscriberCount = Math.max(0, (channels[ci].subscriberCount || 0) + (wasSub ? -1 : 1));
      channels[ci].subscribers = fmtSubs(channels[ci].subscriberCount);
      store.set(KEYS.CHANNELS, channels);
    }
    return { subscribed: !wasSub, count: ci !== -1 ? channels[ci].subscriberCount : 0 };
  }

  // ── VIDEOS ────────────────────────────────────────────────────
  function _allVideos()               { return store.get(KEYS.VIDEOS) || []; }
  function getVideos()                { return _allVideos().filter(v => !v.IsBanned); }
  function getAllVideos()              { return _allVideos(); }
  function getVideo(id)               { return _allVideos().find(v => v.id === id) || null; }
  ...
  function uploadVideoWithId({ id, title, description, tags, category, thumbnail, videoUrl, videoStorageKey, channelId, uploaderId }) {
    ...
    store.set(KEYS.VIDEOS, [..._allVideos(), vid]);
    return { ok: true, video: vid };
  }

  // Permanently removes the video from localStorage.
  // This is the real delete — no tombstone, no flag.
  function deleteVideo(videoId, requesterId) {
    const all = _allVideos();
    const vid = all.find(v => v.id === videoId);
    ...
    if (comments[videoId]) {
      ...
      store.set(KEYS.COMMENTS, comments);
    }
  ...
  function hasLiked(uid, vid)            { return (getLikes()[uid]    || []).includes(vid); }
  function hasDisliked(uid, vid)         { return (getDislikes()[uid] || []).includes(vid); }
  ...
  function toggleLike(userId, videoId) {
    ...
    const wasLiked     = (likes[userId]    || []).includes(videoId);
    const wasDisliked  = (dislikes[userId] || []).includes(videoId);

    if (wasLiked) {
      likes[userId] = (likes[userId] || []).filter(id => id !== videoId);
      if (vi !== -1) videos[vi].likes = Math.max(0, (videos[vi].likes || 0) - 1);
    } else {
      likes[userId] = [...(likes[userId] || []), videoId];
      if (vi !== -1) videos[vi].likes = (videos[vi].likes || 0) + 1;
      if (wasDisliked) {
        dislikes[userId] = (dislikes[userId] || []).filter(id => id !== videoId);
        if (vi !== -1) videos[vi].dislikes = Math.max(0, (videos[vi].dislikes || 0) - 1);
      }
    }
    store.set(KEYS.LIKES, likes);
    store.set(KEYS.DISLIKES, dislikes);
    store.set(KEYS.VIDEOS, videos);
    return { liked: !wasLiked, likes: vi !== -1 ? videos[vi].likes : 0, dislikes: vi !== -1 ? videos[vi].dislikes : 0 };
  }
  ...
  function toggleDislike(userId, videoId) {
    ...
    if (wasDisliked) {
      ...
      if (vi !== -1) videos[vi].dislikes = (videos[vi].dislikes || 0) + 1;
      if (wasLiked) {
        likes[userId] = (likes[userId] || []).filter(id => id !== videoId);
        if (vi !== -1) videos[vi].likes = Math.max(0, (videos[vi].likes || 0) - 1);
      }
    ...
    store.set(KEYS.DISLIKES, dislikes);
    store.set(KEYS.VIDEOS, videos);
    return { disliked: !wasDisliked, likes: vi !== -1 ? videos[vi].likes : 0, dislikes: vi !== -1 ? videos[vi].dislikes : 0 };
  }
  ...
  function addReply(videoId, commentId, { userId, text }) {
    ...
    if (ci === -1) return { ok: false, error: 'Comment not found.' };
    const reply = {
      id: 'r_' + uid(),
      user: user.displayName || user.username,
      avatar: user.avatar,
    ...
    list[ci].replies = [...(list[ci].replies || []), reply];
    store.set(KEYS.COMMENTS, all);
  ...
  function badgeHTML(ch, size = 14) {
    if (!ch) return '';
    const attr  = `width="${size}" height="${size}" viewBox="0 0 24 24" fill="currentColor"`;
    const star  = `<svg ${attr}><path d="M12 2l2.09 6.26L21 9.27l-5 4.87 1.18 6.88L12 17.77l-5.18 3.25L8 14.14 3 9.27l6.91-1.01z"/></svg>`;
    const check = `<svg ${attr}><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>`;
  ...
  function renderTopbar() {
    ...
    const authSection = user ? `
      ...
      </div>` : `<a href="login.html" class="tb-sign-in-btn">Sign In</a>`;

    const uploadBtn = user && userChannel ? `
  ...
  document.addEventListener('click', e => {
    ...
    if (menu && !menu.contains(e.target)) document.getElementById('tbDropdown')?.classList.remove('open');
  });
  ...
  function injectTopbarStyles() {
    ...
    s.textContent = `
      ...
      .tb-dropdown{position:absolute;top:calc(100% + 10px);right:0;width:240px;background:var(--bg2,#0f0f1a);border:1px solid var(--border);border-radius:14px;overflow:hidden;opacity:0;pointer-events:none;transform:translateY(-8px);transition:.2s;z-index:999;box-shadow:0 16px 40px rgba(0,0,0,.6)}
